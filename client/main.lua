...
RegisterNetEvent('RL-autopilot:client:start', function(drivemode)
    local speed = Config.DriveStyles[drivemode].speed
    Start(speed, Config.DriveStyles[drivemode].style)
end)

RegisterNetEvent('RL-autopilot:client:stop', function()
    Stop('stop')
end)

RegisterNetEvent('RL-autopilot:client:menu', function()
    local drivestyles = {}
    ...
    if menu then
        if menu.drivemode then TriggerEvent('RL-autopilot:client:start', menu.drivemode) else return end
    end
end)
